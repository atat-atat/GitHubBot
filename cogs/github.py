...
class GitHubCog:
	...
	async def github_add_repo(self, ctx, repo_owner : str, repo_name : str):
		...
		"""
		async with aiohttp.get('https://api.github.com/repos/{}/{}/commits?per_page=1'.format(repo_owner, repo_name)) as r:
	...
	@github.command(name='del_repo', pass_context=True)
					...
					for channel in git_json["repositories"][server]:
						for repo_owner in git_json["repositories"][server][channel]:
							for repo in git_json["repositories"][server][channel][repo_owner]:
								...
								url = "https://api.github.com/repos/{}/{}/commits?per_page=1".format(repo_owner, repo)
								...
								if repo_owner not in self.repo_data:
									self.repo_data[repo_owner] = response[0]["sha"]
									await self.bot.send_message(discord.Object(channel), fmt)
								elif response[0]["sha"] != self.repo_data[repo_owner]:
									self.repo_data[repo_owner] = response[0]["sha"]
									await self.bot.send_message(discord.Object(channel), fmt)

								issues_url = "https://api.github.com/repos/{}/{}/issues?per_page=1".format(repo_owner, repo)
								...
								if len(response) > 0:
									...
									if repo not in self.repo_issues:
										self.repo_issues[repo] = issue_id
										await self.bot.send_message(discord.Object(channel), fmt)
									elif self.repo_issues[repo] != issue_id:
										self.repo_issues[repo] = issue_id
										await self.bot.send_message(discord.Object(channel), fmt)
